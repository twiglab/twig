...
type (
	...
	// KeyAuthValidator 校验函数
	KeyAuthValidator func(string, twig.Ctx) (bool, error)

	keyExtractor func(twig.Ctx) (string, error)
)

var (
	// DefaultKeyAuthConfig 默认的 KeyAuth 配置.
	DefaultKeyAuthConfig = KeyAuthConfig{
		Skipper:    DefaultSkipper,
		KeyLookup:  "header:" + twig.HeaderAuthorization,
		AuthScheme: "Bearer",
	}
)
...
func KeyAuth(fn KeyAuthValidator) twig.MiddlewareFunc {
...
// See `KeyAuth()`.
func KeyAuthWithConfig(config KeyAuthConfig) twig.MiddlewareFunc {
	// Defaults
	...
	if config.Validator == nil {
		panic("twig. key-auth middleware requires a validator function")
	}
...
func keyFromHeader(header string, authScheme string) keyExtractor {
	return func(c twig.Ctx) (string, error) {
		auth := c.Req().Header.Get(header)
		if auth == "" {
			return "", errors.New("missing key in request header")
		}
		if header == twig.HeaderAuthorization {
			l := len(authScheme)
